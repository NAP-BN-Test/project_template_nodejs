                ...
                try {
                    mtblPointManager(db).findOne({ where: { ID: body.id } }).then(data => {
                        if (data) {
                            var obj = {
                                id: data.ID,
                                totalPoint: data.TotalPoint ? data.TotalPoint : null,
                                idUser: data.IDUser ? data.IDUser : null,
                                plusPoints: data.PlusPoints ? data.PlusPoints : null,
                                minusPoints: data.MinusPoints ? data.MinusPoints : null,
                                reason: data.Reason ? data.Reason : "",
                                idVideoView: data.IDVideoView ? data.IDVideoView : null,
                                viewDuration: data.ViewDuration ? data.ViewDuration : null,
            ...
            if (db) {
                try {
                    mtblPointManager(db).create({
                        ...
                        IDVideoView: body.idVideoView ? body.idVideoView : null,
        ...
        database.connectDatabase().then(async db => {
            if (db) {
                try {
                    ...
                    if (body.idUser || body.idUser === '') {
                        if (body.idUser === '')
                            update.push({ key: 'IDUser', value: null });
                        else
                            update.push({ key: 'IDUser', value: body.idUser });
                    }
                    ...
                    if (body.idVideoView || body.idVideoView === '') {
                        if (body.idVideoView === '')
                            update.push({ key: 'IDVideoView', value: null });
                        else
                            update.push({ key: 'IDVideoView', value: body.idVideoView });
                    }
                    ...
                    mtblPointManager(db).findAll({
                        ...
                        data.forEach(element => {
                            var obj = {
                                ...
                                totalPoint: element.TotalPoint ? element.TotalPoint : null,
                                idUser: element.IDUser ? element.IDUser : null,
                                plusPoints: element.PlusPoints ? element.PlusPoints : null,
                                minusPoints: element.MinusPoints ? element.MinusPoints : null,
                                reason: element.Reason ? element.Reason : "",
                                idVideoView: element.IDVideoView ? element.IDVideoView : null,
                                viewDuration: element.ViewDuration ? element.ViewDuration : null,
                                createDate: element.CreateDate ? element.CreateDate : null,
                                editDate: element.EditDate ? element.EditDate : null,
                                startTime: element.StartTime ? element.StartTime : null,
                                endTime: element.EndTime ? element.EndTime : null,
                            }
                            array.push(obj);
                            stt += 1;
                        });
